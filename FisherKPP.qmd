...
This implementation of numerical solve of reaction diffusion equation is based on the [presentation](https://www.youtube.com/watch?v=8gLhaWRYvfQ) of the package `MethodOfLines.jl` at JuliaCon 2022 by A. Jones.
...
Let us first import the parameters used for the simulation
...
`MethodsOfLines.jl` makes use of `ModelllingToolkit.jl` to symbolically define the model to integrate. 
...
We set up the discretization of the state space, through `MethodOfLines.jl`:
